...
-- pusher
push={y=34,h=10,min_y=32,max_y=48,
...
function update_play()
 ...
 -- end round: met target or out of coins
 local can_end=round_score>=target
  or (coins_left<=0 and #dropping==0)

 -- left/right: select inventory
 -- (end round is last slot when available)
 local max_sel=#inv+(can_end and 1 or 0)
 if btnp(0) then
  inv_sel=max(1,inv_sel-1)
 end
 if btnp(1) then
  inv_sel=min(max_sel,inv_sel+1)
 end
 ...
 if btnp(5) then
  if can_end and inv_sel==#inv+1 then
   finish_round()
   return
  elseif inv_sel<=#inv and inv_sel>0 then
 ...
 if btnp(3) and gold>=buy_coin_cost then
  gold-=buy_coin_cost
  buy_count+=1
  buy_coin_cost=buy_coin_base+buy_count*buy_coin_base
...
function update_coins()
 ...
 -- spatial grid collision (8px cells)
 -- bucket coins into 14x6 grid
 local grid={}
 local nbr={1,13,14,15}
 for i=1,#coins do
  local c=coins[i]
  local k=mid(0,flr((c.y-ft)/8),5)*14
   +mid(0,flr((c.x-fl)/8),13)
  if grid[k] then add(grid[k],c)
  else grid[k]={c} end
 end
 -- check same-cell + 4 forward neighbors
 for k,cell in pairs(grid) do
  local cn=#cell
  local gx=k%14
  for i=1,cn do
   for j=i+1,cn do
    collide_coins(cell[i],cell[j])
   end
  end
  for n=1,4 do
   local nk=k+nbr[n]
   if abs(nk%14-gx)<=1 then
    local nb=grid[nk]
    if nb then
     for i=1,cn do
      for j=1,#nb do
       collide_coins(cell[i],nb[j])
      end
     end
    end
   end
...
function draw_bottom_panel()
 rectfill(0,76,127,127,0)

 -- progress bar (y=76-81)
 rectfill(1,76,126,81,1)
 local bw=min(124,
  flr(124*round_score/target))
 if bw>0 then
  local bc=round_score>=target and 11 or 8
  rectfill(2,77,2+bw,80,bc)
 end
 local stxt=round_score.."/"..target
 print(stxt,64-#stxt*2,77,7)

 local can_end=round_score>=target
  or (coins_left<=0 and #dropping==0)
 local end_sel=can_end
  and inv_sel==#inv+1

 -- inventory slots (y=83-94)
 print("\f6\x83\x84",1,84)
 for i=1,inv_max do
  local sx=10+(i-1)*22
  local sy=83
  local sel=(i==inv_sel and not end_sel)
 ...
 -- desc area (y=96)
 if inv_sel>0 and inv_sel<=#inv
    and not end_sel then
  local item=inv[inv_sel]
  local nm=spc_names[item.type]
  print(nm,2,97,spc_cols[item.type])
  print(spc_descs[item.type],
   #nm*4+4,97,6)
 end

 -- end round button (y=105-113)
 if can_end then
  local bcol=end_sel and 11 or 6
  local bg=end_sel and 3 or 1
  rectfill(2,105,62,113,bg)
  rect(2,105,62,113,bcol)
  print("end round\x91",6,108,bcol)
 end

 -- buy coins (y=119)
 local gcol=gold>=buy_coin_cost
  and "\fa" or "\f8"
 print("\f6\x83:buy "..buy_coin_amt
  .." coins "..gcol.."g"..buy_coin_cost,2,119)
 if round_score>=target and t%40<25 then
  print("\fbtarget!",96,108)
 end
end
...
__label__

...

...

...

...

...

...

...
__sfx__

